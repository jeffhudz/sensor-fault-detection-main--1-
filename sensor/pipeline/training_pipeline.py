...
class TrainPipeline:
    ...
    def syc_artifact_dir_to_s3(self,):
        try:
            aws_bucket_url = f"s3://{TRAINING_BUCKET_NAME}/artifact/{self.training_pipeline_config.timestamp}"
            S3Sync.sync_folder_to_s3(folder =self.training_pipeline_config.artifact_dir,aws_bucket_url=aws_bucket_url)
        except  Exception as e:
            raise  SensorException(e,sys)
        
    def syc_saved_model_dir_to_s3(self):
        try:
            aws_bucket_url = f"s3://{TRAINING_BUCKET_NAME}/{SAVED_MODEL_DIR}"
            S3Sync.sync_folder_to_s3(folder =SAVED_MODEL_DIR,aws_bucket_url=aws_bucket_url)
        except  Exception as e:
            raise  SensorException(e,sys)
    ...
    def run_pipeline(self):
        try:
            ...
            self.syc_artifact_dir_to_s3()
            self.syc_saved_model_dir_to_s3()
        except  Exception as e:
            TrainPipeline.is_pipeline_running=False
            raise  SensorException(e,sys)